...
main = do
 ...
 print "Result"
 printMaze (onePlayerManyMoves maze moves)


-- YOUR CODE SHOULD COME AFTER THIS POINT
onePlayerManyMoves :: [[Char]] -> [Char] -> [[Char]]
onePlayerManyMoves maze [] = maze
onePlayerManyMoves maze (h:t) = onePlayerManyMoves newMaze t
 where
 origMaze = maze
 newMaze = checkIfExitFound maze origMaze h


checkIfExitFound :: [[Char]] -> [[Char]] -> Char -> [[Char]]
checkIfExitFound maze origMaze move
 | (playerAtExit == 1) = maze
 | otherwise = (onePlayerOneMove maze move)
 where
 playerAtExit = exitFound maze
...
exitFound maze
 ...
 | otherwise = 0
 where
 player = head(find2D '1' maze)
 exit = findExit maze 0 '1'


onePlayerOneMove :: [[Char]] -> Char -> [[Char]]
onePlayerOneMove maze move = finalMaze
 where
 player = head(find2D '1' maze)
 adj = getAdjacent player
 nextCords = (newDirectionPlus move adj)
 exit = findExit maze 0 '1'
 isNextClear = recurseNext maze player nextCords exit move '1'
 xPlayer = fst player
 yPlayer = snd player
 col = mazeCols maze
 row = mazeRows maze
...
findExit :: [[Char]] -> Int -> Char -> (Int,Int)
findExit maze i player
 | ((check == '-') || (check == player)) = pair
 | otherwise = findExit maze (i+1) player
