...
main = do
 ...
 print "Result"
 printMaze (manyPlayersOneMove maze moves)


-- YOUR CODE SHOULD COME AFTER THIS POINT
manyPlayersOneMove :: [[Char]] -> [Char] -> [[Char]]
manyPlayersOneMove maze moves = newMaze
 where
 flatmaze = collapse1D maze
 intialListPlayers = []
 players = sort(findAllPlayers flatmaze intialListPlayers 0)
 newMaze = runEachPlayerOnce maze moves players 0


runEachPlayerOnce :: [[Char]] -> [Char] -> [Char] -> Int -> [[Char]]
runEachPlayerOnce maze moves players i
 | ((i < ((length players) -1)) && (i < ((length moves) -1))) = runEachPlayerOnce newMaze moves players (i+1)
 | otherwise = newMaze
 where
 currMove = moves!!i
